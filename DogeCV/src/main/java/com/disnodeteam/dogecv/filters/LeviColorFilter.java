    ...
    public void process(Mat input, Mat mask) {
        ...
        switch(color){
            case RED:
                ...
                Imgproc.threshold(channels.get(1), mask, threshold, 300, Imgproc.THRESH_BINARY);
                break;
            case BLUE:
                if(threshold == -1){
                    threshold = 145;
            ...
            case WHITE:
                ...
                Imgproc.GaussianBlur(input,input,new Size(3,3),0);
                Core.split(input, channels);
                Core.inRange(channels.get(0), new Scalar(threshold, 150, 40), new Scalar(300, 150, 150), mask);
                break;
            case YELLOW:
                if(threshold == -1){
                    threshold = 70;
                ...
                Core.split(input, channels);
                if(channels.size() > 0){
                    Imgproc.threshold(channels.get(1), mask, threshold, 300, Imgproc.THRESH_BINARY_INV);
                }
